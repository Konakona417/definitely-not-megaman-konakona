...
public class GameLogic {
	...
	private List<BulletEnemy> enemyBullets;
	...
	public void checkConditions(){
		...
		if(!status.isGameWon()) {
			System.out.println("Boom is " + gameScreen.getBoom() + " and level is " + status.getLevel());
			if (gameScreen.getBoom() >= 2 && status.getLevel() == 1) {
				System.out.println("WEEE");
				gameWon();
			}

			else if(gameScreen.getBoom() >= 8 && status.getLevel()==2)
				gameWon();
		}
	}
	...
	public boolean moveEnemyBullet(BulletEnemy bullet){
		if(bullet.getY() - bullet.getSpeed() >= 0){
			bullet.translate(bullet.getSpeed(), 0);
			return false;
		}
	...
	//Background Brinstar
	public Background brinstar(GameScreen screen){
		this.background = new Background(screen);
		return background;}
	/**
