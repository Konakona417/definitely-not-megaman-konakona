...
import rbadia.voidspace.model.Background;
...
public class GameScreen extends BaseScreen{
	...
	private long lastAsteroidTime, lastAsteroidTime2, asteroidTiming = System.currentTimeMillis(), asteroidTiming2 = System.currentTimeMillis();
	...
	public void updateScreen(){
		Background brinstar = gameLogic.getBackground();
		Background gloombrinstar = gameLogic.getBackground();
		...
		if (status.getLevel() == 1){
			graphicsMan.drawBackground(brinstar, g2d, this);
		}
		if (status.getLevel() == 2){
			graphicsMan.drawgloomBackground(gloombrinstar, g2d, this);
		}
		if (status.getLevel() == 3){
			graphicsMan.drawvariaBackground(gloombrinstar, g2d, this);
		}
		//draw Floor
		for(int i=0; i<9; i++){
		...
		if (lastAsteroidTime == -500) lastAsteroidTime = System.currentTimeMillis();

		// draw first asteroid
		if(!status.isNewAsteroid() && status.getLevel() == 1){
			// draw the asteroid until it reaches the bottom of the screen

			//LEVEL 1
			if((asteroid.getX() + asteroid.getAsteroidWidth() >  0) && (boom <= 5 || boom == 15)){
				long diff = (System.currentTimeMillis() - asteroidTiming)/100;
				asteroid.translate((int) (-asteroid.getSpeed()-diff*diff*0.3), 0);
				graphicsMan.drawAsteroid(asteroid, g2d, this);
			}
			else if (boom <= 5){
				asteroidTiming = System.currentTimeMillis();
				asteroid.setLocation(this.getWidth() - asteroid.getAsteroidWidth(),
						rand.nextInt(this.getHeight() - asteroid.getAsteroidHeight() - 32));
			}	
		}

		else if(!status.isNewAsteroid() && status.getLevel() == 2){
			// draw the asteroid until it reaches the bottom of the screen
			//LEVEL 2
			if((asteroid.getX() + asteroid.getAsteroidWidth() >  0)){
				long diff = (int)(System.currentTimeMillis() - asteroidTiming)/100;
				asteroid.translate((int)(-asteroid.getSpeed()-diff*diff*0.3), (int)(asteroid.getSpeed()+diff*diff*0.3));
		 		graphicsMan.drawAsteroid(asteroid, g2d, this);
			}
			else if (boom <= 5) {
				asteroid.setLocation(this.getHeight() - asteroid.getAsteroidHeight(),
						rand.nextInt(this.getHeight() - asteroid.getAsteroidHeight() - 32));

			}
		}

		else if(!status.isNewAsteroid() && status.getLevel() == 3){
			// draw the asteroid until it reaches the bottom of the screen
			//LEVEL 3
			if((asteroid.getY() - asteroid.getAsteroidHeight() >  0)){
				long diff = (int)(System.currentTimeMillis() - asteroidTiming)/100;
				asteroid.translate(0, (int)(asteroid.getSpeed()+diff*diff*0.3));
				graphicsMan.drawAsteroid(asteroid, g2d, this);
			}
			else if (boom <= 15) {
				asteroid.setLocation(rand.nextInt(this.getWidth() - asteroid.getAsteroidWidth()-32), 33);
			}
			...
			if((currentTime - lastAsteroidTime) > NEW_ASTEROID_DELAY){
				...
				asteroidTiming = System.currentTimeMillis();
				if (status.getLevel() == 3) {
					asteroid.setLocation(rand.nextInt(this.getWidth() - asteroid.getAsteroidWidth()), 0);
				} else
				asteroid.setLocation(this.getWidth() - asteroid.getAsteroidWidth(),
						rand.nextInt(this.getHeight() - asteroid.getAsteroidHeight() - 32));
			}
		...
		if (lastAsteroidTime2 == -500) lastAsteroidTime2 = System.currentTimeMillis();
		
		if(!status.isNewAsteroid2() && status.getLevel() == 1){
			// draw the asteroid2 until it reaches the bottom of the screen

			//LEVEL 1
			if((asteroid2.getX() + asteroid2.getAsteroidWidth() >  0) && (boom <= 5 || boom == 15)){
				long diff = (System.currentTimeMillis() - asteroidTiming2)/100;
				asteroid2.translate((int) (-asteroid2.getSpeed()-diff*diff*0.3), 0);
				graphicsMan.drawAsteroid(asteroid2, g2d, this);
			}
			else if (boom <= 5){
				asteroidTiming2 = System.currentTimeMillis();
				asteroid2.setLocation(this.getWidth() - asteroid2.getAsteroidWidth(),
						rand.nextInt(this.getHeight() - asteroid2.getAsteroidHeight() - 32));
			}	
		}

		else if(!status.isNewAsteroid2() && status.getLevel() == 2){
			// draw the asteroid2 until it reaches the bottom of the screen
			//LEVEL 2
			if((asteroid2.getX() + asteroid2.getAsteroidWidth() >  0)){
				long diff = (int)(System.currentTimeMillis() - asteroidTiming2)/100;
				asteroid2.translate((int)(-asteroid2.getSpeed()-diff*diff*0.3), (int)(asteroid2.getSpeed()+diff*diff*0.3));
				graphicsMan.drawAsteroid(asteroid2, g2d, this);
			}
			else if (boom <= 5){
				asteroidTiming2 = System.currentTimeMillis();
		...
		else if(!status.isNewAsteroid2() && status.getLevel() == 3){
			// draw the asteroid2 until it reaches the bottom of the screen
			//LEVEL 3
			if((asteroid2.getY() - asteroid2.getAsteroidHeight() >  0)){
				long diff = (int)(System.currentTimeMillis() - asteroidTiming2)/100;
				asteroid2.translate(0, (int)(asteroid2.getSpeed()+diff*diff*0.3));
				graphicsMan.drawAsteroid(asteroid2, g2d, this);
			}
			else if (boom <= 15){
				asteroidTiming2 = System.currentTimeMillis();
				asteroid2.setLocation(rand.nextInt(this.getWidth() - asteroid2.getAsteroidWidth()-32), 33);
				//asteroid2.setLocation(this.getWidth() - asteroid2.getAsteroidWidth(),
				//		rand.nextInt(this.getHeight() - asteroid2.getAsteroidHeight() - 32));
			...
			if((currentTime - lastAsteroidTime2) > NEW_ASTEROID_DELAY){
				...
				asteroidTiming2 = System.currentTimeMillis();
				if (status.getLevel() == 3) {
					asteroid2.setLocation(rand.nextInt(this.getWidth() - asteroid2.getAsteroidWidth()), 0);
				} else
				asteroid2.setLocation(this.getWidth() - asteroid2.getAsteroidWidth(),
						rand.nextInt(this.getHeight() - asteroid2.getAsteroidHeight() - 32));
			}
		...
		if (!nPress && input.getN()){
			if (nPress && !input.getN()) nPress = false;
			if (status.getLevel()==1){
				restructure();
			}
			...
			if (status.getLevel()==1 && boom >= 2 && gameLogic.getStatus().isGameWon()){
				restructure();
			} else if (status.getLevel()==2 && boom >= 8){
				thirdlevel();
			}
		}
	...
	public void thirdlevel(){
		Platform[] platform = gameLogic.getNumPlatforms();
		for(int i=0; i<8; i++){
			if(i<4)	platform[i].setLocation(80 + i*40, getHeight()/2);
			if(i==4) platform[i].setLocation(50 +i*50, getHeight()/2 + 140 - 3*40);
			if(i>4){	
				int n=4;
				platform[i].setLocation(50 + i*50, getHeight()/2 + 20 + (i-n)*40 );
